...
> [!NOTE]
> This repo keeps the whole template in a single `main.tf` by design (it is one small Coder template). The multi-file split below is the convention for larger configurations — adopt it only if `main.tf` grows unwieldy.
...
> [!NOTE]
> In this Coder template, `count = data.coder_workspace.me.start_count` on the container and modules is the idiomatic use — it creates the resource only when the workspace is started. That is a legitimate `count` use, not an anti-pattern.
...
> [!NOTE]
> This repo's `.gitignore` already covers all of the above. Keep it that way.
